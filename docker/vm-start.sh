...
# Mount Azure File Share if not already mounted (fstab handles it at boot)
mkdir -p /mnt/kunkka-db-files
if ! mountpoint -q /mnt/kunkka-db-files; then
  if [ -f /etc/kunkka-storage.key ]; then
    ...
    echo "WARNING: /etc/kunkka-storage.key not found, skipping manual mount"
  fi
else
  echo "File share already mounted at /mnt/kunkka-db-files"
fi
...
docker run -d --name kunkka-torrent --restart unless-stopped \
  -p 80:80 -p 443:443 -p 6881:6881/tcp -p 6881:6881/udp -p 6882:6882/udp \
